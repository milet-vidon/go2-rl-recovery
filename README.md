...
真实倒地训练已进入独立的 Bank 试验：256个候选状态中244个通过落地和冷重放验证，214个训练、30个独立保留；37项真实仿真重置检查通过后，已启动新的1000轮训练。数据通过验收不等于机器人已经会起身；旧策略在这30个保留状态上仍未恢复，新权重尚未晋升。[训练参数与来源](configs/recovery-bank-pilot-20260916/) · [状态库与验证记录](datasets/recovery_states/nominal_pd_v1_20260916/manifest.json)。
