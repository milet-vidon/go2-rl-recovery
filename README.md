...
**最新连续演示（2026-09-17，整体目标尚未完成）：** [倒地恢复 → 站稳 → 低速行走 → 停止，正面＋斜侧面](evaluations/20260917-continuous05-balanced4246-side-paired-v3/continuous_flow_front_oblique.mp4)。20.1秒未剪辑模拟实录，包含初始1秒PD准备；两视角是轨迹逐步一致的独立回放，不是同时拍摄。单个状态库侧倒初态、0.5m/s指令通过17项检查和逐步记录审计，运动均速0.543m/s。起身撑腿仍不够自然；这不是运动中跌倒测试，也不含小跑／快跑。[实际视觉检查、原始报告与局限](docs/continuous-flow-video-review-20260917.md)。

姿态调整分支在普通站立、侧倒和仰倒各20次开发集复测保住片尾严格站立，左右落脚偏差减小，但前后对齐仍不足；它尚未接入上述连续演示。[姿态结果](docs/supported-refinement-results-20260917.md)。等训练量的步态对照已完成：两组宽范围回归仅11/21、13/21，占空差改善12.35%，未达到预声明晋级标准；不替换保留模型。[全部对照及负结果](docs/balanced-gait-results-20260917.md)。下一轮独立减滑移对照与动物示范数据路线正在研究；速度达标不等于自然跑姿。[当前任务状态](docs/training-status-20260916.md) · [公开数据与许可审查](docs/public-animal-motion-data-review-20260917.md)。以下导出模型和历史证据继续保留。

[本次 GitHub 发布快照与范围](docs/github-snapshot-20260917.md) · [开发模型、来源与使用限制](models/development/20260917/README.md)。公开开发权重不等于模型晋级或实机可部署。

**历史已验证成果：平地静止支撑、0.5 m/s 指令下的对角支撑行走、停止后回到四足站立，并通过横向速度冲击测试。** 这里的对角支撑测量不构成自然小跑（trot）已实现的证明。保留的旧走停协议推荐模型是 `natural_stop_diagonal_model2250.pt`：它在三组随机种子下通过固定走停测试，并在 `0.5 m/s` 横向速度冲击下无摔倒、无机身触地。另有推扰强化模型 `model_3648.pt`，在三组种子下通过无扰、左右转向和高速走停回归；`0.75 m/s` 推扰后仍会短时卸载足端，因此只作为实验候选，不替代推荐模型。此前 Natural Stop 1349 和 Natural 950 的结果也完整保留。仍有轻微倾斜和足端轨迹差异；没有完成参考视频全部行为、AMP 复现或实机验证。
...
姿态专用复测（仅恢复与静止姿态调整，不含行走／跑动）：[侧倒：正面＋斜侧面](evaluations/20260917-refinement20-side-paired/supported_refinement_side_front_oblique.mp4) · [仰倒：正面＋斜侧面](evaluations/20260917-refinement20-upside_down-paired/supported_refinement_upside_down_front_oblique.mp4) · [视觉检查与局限](docs/refinement-video-visual-review-20260917.md)。它与页首连续低速流程是不同实验，不合并成功率或冒充一段连续控制。

![站立、行走、停止的连续仿真关键帧](docs/images/stop1349_stance.jpg)

**继续训练（2026-09-16，整体未达标）：** 保留正常站姿复习的20–60°恢复课程已完成1000轮、12,288,000新增环境步。最终4447通过30°各类20/20；45°侧倾15/20、前后倾18/20，低于中间3900的16/20和20/20。先落地稳定再接管时仍然侧躺0/18、仰躺0/20，因此没有晋升新模型；下一步针对真实倒地状态训练。行走2250通过加强后的站立/停止几何与四脚垂直支撑检查。[当前进度、失败证据与续训验收](docs/training-status-20260916.md)。

## 演示与测量

本节2026-09-16及更早的条目是历史记录，当时的“已启动／待测”不代表当前运行状态；当前进程与最新结论以上方2026-09-17进度页为准。尚未交付通过验收的恢复、行走和快跑一体化视频。

2026-09-16 20:18：Smith-inspired2000轮已完成，九个20次姿态组仍均未站稳，不能晋升。视频确认能从背躺翻到机身朝上，但随后仍低趴、腿部支撑异常；已录制并实际检查新的正面＋斜侧面视频。[最新实录与失败证据](docs/smith-video-review-20260916.md)。目前已启动两组各1000轮的续训对照：同一父模型，只有翻正后的站起奖励权重10/30不同；配置和短测通过，正式结果待测。[下一轮设计](docs/smith-standweight-pair-20260916.md) · [运行状态](docs/training-status-20260916.md)。

2026-09-16：128环境动作参考对照的两组均已完成，各2000轮、6,144,000环境步；双方在已落地侧躺／背躺、30°直立／侧倾／前后倾、45°直立／侧倾／前后倾／背躺共9个姿态组中，success、final-valid、final-geometry 均各为0/20。不晋升任一候选，也不原样延长训练。此前512环境训练曾因显存不足中断，其历史记录保留，但本次完整结果已替代“第二组仍在训练”的进度；两轮不能混作同一实验。现有正常走停与有限倾角恢复模型保持不变。最新正面＋斜侧面完整实录已交付（118.16秒）：正常走停通过，冲击后无法恢复，两组新模型均失败；已实际检查所有场景与片尾。不同策略分别标注，不包装为尚未实现的连续闭环。[完整对照与负结果](docs/action-reference-ablation-20260916.md) · [最新视频核对与交付进度](docs/video-review-20260916.md)。

真实倒地 Bank 试验已完成1000轮，但最终4899仍未通过：侧躺0/20（16条独立保留状态）、背躺0/20（14条独立状态）；30°侧倾也退步至10/20，因此不晋升、不覆盖已有推荐模型。随后两组各100轮站姿惩罚对照、另100轮背躺条件探索也未带来真实倒地恢复；这些失败与回归完整保留，不能声称已复现参考视频。状态库244条通过物理验证只表示初态有效，不表示恢复成功；反复用于调参的保留集也不是最终盲测集。[奖励对照](docs/recovery-gate-ablation-20260916.md) · [探索试验](docs/back-exploration-20260916.md) · [状态库验证](datasets/recovery_states/nominal_pd_v1_20260916/manifest.json)。
...
历史模型分别验证了平地、固定正向速度、连续转向、高速走停及有限横向冲击，不代表最新组合在这些场景全部通过。Aligned 3448在单种子的 upright 与30°受控侧倾/前后倾中各通过20/20，正面视频中前腿不交叉；仍有轻微左右差异，该模型尚未证明更大倾角、预先静止倒地、倒置和任意初始姿态的可靠恢复。最新已完成 roll1999 → stand3547 → balanced4246 的单个状态库侧倒初态、0.5 m/s 行走与停止连续切换诊断；恢复与运动仍由不同模型承担，姿态调整模型3746尚未接入该流程。这不代表任意初态恢复、自然小跑／快跑或参考视频全部行为已复现。复杂地形、AMP与实机部署也未完成。历史恢复报告缺少逐腿几何检查，不能作为正常站姿证明；3498保留为交叉腿失败对照。
...
历史 `robust_model799.pt` 存在塌陷，`standard_stance_model950.pt` 是早期实验，均不是本页的新 Natural 950。旧视频保留用于比较，不作为成功演示。历史独立恢复模型曾在 30° 初始姿态测试中得到前后倾 17/20、侧倾 1/20；这些旧试验未解决大侧翻和倒置恢复，其成绩不属于行走模型，也不替代页首新组合的独立证据。
