...
2026-09-16 16:14 已启动128环境的动作参考对照训练，两组各2000轮（每组614.4万环境步），顺序运行并严格复测，尚无正式结果。此前512环境训练因显存不足在584轮附近中断，保存的500轮模型仍然侧躺／背躺各0/20；不能称作已完成或已恢复。正常行走及有限倾角恢复模型保持不变，同时按论文与实际源码核对后续方向。[实验与进度](docs/action-reference-ablation-20260916.md)。
