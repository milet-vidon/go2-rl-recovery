...
真实倒地 Bank 试验已完成1000轮，但最终4899仍未通过：侧躺0/20（16条独立保留状态）、背躺0/20（14条独立状态）；30°侧倾也退步至10/20，因此不晋升、不覆盖已有推荐模型。侧躺结束时平均仍倾斜65.7°、仅一足支撑。下一项有界对照将检验“翻身途中提前施加站姿惩罚”的影响；严格站姿验收不变。状态库244条通过物理验证，只表示初态有效，不表示恢复成功。[训练参数与来源](configs/recovery-bank-pilot-20260916/) · [状态库与验证记录](datasets/recovery_states/nominal_pd_v1_20260916/manifest.json)。
