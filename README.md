...
2026-09-16：128环境动作参考对照的第一组已完成2000轮（614.4万环境步），但侧躺／背躺均0/20，直立及30/45度各姿态也未通过严格站姿验收；第二组仍在训练。此前512环境训练曾因显存不足中断，该轮与本次不能混作同一实验。现有正常走停与有限倾角恢复模型保持不变。完整复测后优先提供本轮双视角失败或成功实录，明确区分走停基线与新恢复策略，不包装为尚未实现的连续闭环。[实验、负结果与视频交付进度](docs/action-reference-ablation-20260916.md)。
