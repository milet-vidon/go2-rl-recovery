...
2026-09-16 15:52 已启动新的动作参考对照训练：相同全新初始权重、相同倒地状态，两组各1000轮，分别使用默认关节位置和当前关节位置作为控制目标参考。两组顺序运行并自动执行严格复测，尚无正式结果，不覆盖现有推荐模型。[实验与进度](docs/action-reference-ablation-20260916.md)。
