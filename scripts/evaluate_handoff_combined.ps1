...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_handoff_combined.py'
...
$taskArgs = @('-B',$taskEntry,'--task',$taskName,'--checkpoint',$Checkpoint,
    '--stand_checkpoint',$StandCheckpoint,'--output_dir',$OutputDir,'--startup_mode',$Mode,'--mirror_mode',$MirrorMode,
...
$taskInvocation = @{schema='handoff_combined_invocation_v1';started_at=(Get-Date).ToString('o');
...
if ($taskReport.protocol_version -ne 'handoff_combined_startup_mirror_experimental_v1' -or
