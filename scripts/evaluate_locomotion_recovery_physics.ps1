...
# Finite frozen-policy diagnostic. Never trains, promotes, or overwrites evidence.
...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_locomotion_recovery_physics.py'
...
$taskPaths = @($PSCommandPath,$taskEntry,$taskTemplate,$taskVerifier,$taskCheckpoint,
...
$taskInvocation = [ordered]@{schema='locomotion_recovery_physics_invocation_v1';status='running';
    ...
    notice='Frozen retention diagnostic only. Original-mode equivalence requires separate baseline comparison; no full-flow or fast-running claim.'}
...
try {
    ...
    if ($taskReport.protocol_version -ne 'locomotion_recovery_physics_retention_v1' -or
        ...
        $taskExperiment.physics_mode -ne $PhysicsMode -or $taskExperiment.frozen_control_sha256 -ne $taskCheckpointSha -or
    ...
    if ($taskTrace.schema -ne 'locomotion_recovery_physics_retention_v1' -or $taskTrace.physics_mode -ne $PhysicsMode -or
