...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_handoff_supported_startup.py'
...
$taskArgs = @('-B',$taskEntry,'--task',$taskName,'--checkpoint',$Checkpoint,
    '--stand_checkpoint',$StandCheckpoint,'--output_dir',$OutputDir,'--startup_mode',$Mode,'--transition_mode','hard',
...
$taskInvocation = @{schema='handoff_supported_startup_invocation_v1';started_at=(Get-Date).ToString('o');
    mode=$Mode;ramp_seconds=0;pose=$Pose;trials=$Trials;seed=$Seed;args=$taskArgs;
...
if ($taskReport.protocol_version -ne 'handoff_supported_startup_experimental_v1' -or
...
$taskGenerated=Join-Path $OutputDir 'supported_startup_generated.py'
...
Write-Output "DIAGNOSTIC COMPLETE startup=$Mode pose=$Pose final=$($taskReport.results.$Pose.final_valid_stands)/$Trials. Not acceptance or promotion."
