"""EXPERIMENTAL hard/raw-ramp two-actor handoff diagnostic; never training collection or acceptance.

Standalone snapshot of frozen evaluate_go2_recovery.py. The original collector,
gate, resets, physics, observations and standing criterion remain unchanged.
Linear raw-action interpolation is an engineering diagnostic, not paper reproduction.
"""
...
def _validate_transition_cli(args):
    ...
    if args.transition_mode == "hard" and args.ramp_seconds != 0:
        raise ValueError("hard requires --ramp_seconds 0")
    if args.transition_mode == "raw_ramp" and args.ramp_seconds not in (0.2, 0.5):
        raise ValueError("raw_ramp requires --ramp_seconds 0.2 or 0.5")
...
parser.add_argument("--ramp_seconds", required=True, type=float, choices=(0.0, 0.2, 0.5))
...
def main() -> None:
    ...
    transition_path = args_cli.output_dir / f"{checkpoint_name}_handoff_transition_trace.json"
    ...
    report["protocol_version"] = "handoff_transition_experimental_v1"
    report["controller_type"] = "experimental_dual_policy_handoff_transition"
