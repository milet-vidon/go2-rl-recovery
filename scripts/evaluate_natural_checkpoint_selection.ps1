param(
    ...
    [Parameter(Mandatory=$true)][string]$CandidateTrainingResult,
    ...
    [ValidateSet('upright','side','upside_down')][string]$Pose = 'upside_down',
...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_natural_checkpoint_selection.py'
...
foreach ($taskInput in @($Checkpoint,$StandCheckpoint,$CandidateTrainingResult,$taskEntry,$taskOldEntry)) {
...
$CandidateTrainingResult = (Resolve-Path -LiteralPath $CandidateTrainingResult).Path
$taskSelectionRun = 'E:\IsaacLab\repo\logs\rsl_rl\unitree_go2_handoff_stand\20260917-natural-stand128x200-first'
if ($CandidateTrainingResult -ne (Join-Path $taskSelectionRun 'training_result.json') -or
    (Get-FileHash -LiteralPath $CandidateTrainingResult -Algorithm SHA256).Hash -ne '531fde006c9f164f7535f0937f7a2828935eb868ed2f39cc00991fe49bdbb94f') {
    throw 'Only the immutable completed formal200 training receipt is allowed.'
}
$taskSelectedName = Split-Path $StandCheckpoint -Leaf
if ((Split-Path $StandCheckpoint -Parent) -ne $taskSelectionRun -or $taskSelectedName -notin @('model_3600.pt','model_3700.pt')) {
    throw 'Only existing same-run3600/3700 checkpoints are permitted; no unbounded scan.'
}
$taskSelectedIteration = if ($taskSelectedName -eq 'model_3600.pt') {3600} else {3700}
$taskEffectiveUpdates = $taskSelectedIteration - 3547 + 1
$taskSelectedAdam = 71120 + 20 * $taskEffectiveUpdates
$taskSelectedSha = if ($taskSelectedIteration -eq 3600) {'a291f537c03dd61d5091134469cf05754dbae5aeade4ad138cb6a2b05e779208'} else {'e7990d5f24a7f8da1dacf1c37c6d4b4db89d59977c57efb4f7c0ccb98ec346ea'}
if ((Get-FileHash -LiteralPath $StandCheckpoint -Algorithm SHA256).Hash -ne $taskSelectedSha) {throw 'Selected checkpoint bytes changed.'}
...
$taskArgs = @('-B',$taskEntry,'--task',$taskName,'--checkpoint',$Checkpoint,
    '--candidate_training_result',$CandidateTrainingResult,'--stand_checkpoint',$StandCheckpoint,'--output_dir',$OutputDir,'--startup_mode',$Mode,'--mirror_mode',$MirrorMode,
...
$taskPaths = @($PSCommandPath,$taskEntry,$taskOldEntry,$Checkpoint,$StandCheckpoint,$CandidateTrainingResult,
    (Join-Path $PSScriptRoot 'evaluate_natural_handoff_candidate.py'),
    (Join-Path $PSScriptRoot 'evaluate_natural_handoff_candidate.ps1'),
    (Join-Path $PSScriptRoot 'test_natural_checkpoint_selection.py'),
    (Join-Path $PSScriptRoot 'train_handoff_stand.py'),
    (Join-Path $PSScriptRoot 'train_natural_handoff_stand.py'),
    (Join-Path $PSScriptRoot 'natural_stance_reward.py'),
    (Join-Path $taskSelectionRun 'model_3746.pt'),
    (Join-Path $taskSelectionRun 'preflight.json'),(Join-Path $taskSelectionRun 'before_learning.json'),
    (Join-Path $taskSelectionRun 'params\env.yaml'),(Join-Path $taskSelectionRun 'params\agent.yaml'),
...
$taskInvocation = @{schema='natural_stance_checkpoint_selection_invocation_v1';started_at=(Get-Date).ToString('o');
    mode=$Mode;mirror_mode=$MirrorMode;ramp_seconds=0;pose=$Pose;trials=$Trials;seed=$Seed;args=$taskArgs;
    source_and_inputs=$taskSnapshot;selection_kind='retrospective_development_selection';effective_updates=$taskEffectiveUpdates;additional_training_updates=0;
    notice='Existing checkpoint selection after final3746 regression, NOT new training/final receipt, fresh generalization or promotion.'}
...
if ($taskReport.protocol_version -ne 'natural_stance_checkpoint_selection_v1' -or
...
if ($taskReport.candidate_training.checkpoint_sha256 -ne 'f40772c228996400a9813ad244b48509737dabf2c31af001144de34283433b9e' -or
    $taskReport.candidate_training.training_receipt_sha256 -ne (Get-FileHash -LiteralPath $CandidateTrainingResult).Hash -or
    $taskReport.candidate_training.parent_checkpoint_sha256 -ne '5527c4053cb6ef8e6616da65b7dc32a9464eb8d1366f4966a4eb4b65bc913fdb' -or
    $taskReport.candidate_training.verified_updates -ne 200 -or
    $taskReport.candidate_training.actual_checkpoint_iteration -ne 3746 -or
    $taskReport.candidate_training.actual_adam_step -ne 75120 -or
    $taskReport.candidate_training.formal_training_verified -ne $true) {throw 'Original formal200/final3746 evidence changed.'}
$taskSelection = $taskReport.checkpoint_selection
if ($taskSelection.protocol -ne 'natural_stance_checkpoint_selection_v1' -or
    $taskSelection.selection_kind -ne 'retrospective_development_selection' -or
    $taskSelection.retrospective_development_selection -ne $true -or
    $taskSelection.checkpoint_sha256 -ne $taskSelectedSha -or
    [IO.Path]::GetFullPath($taskSelection.checkpoint) -ne $StandCheckpoint -or
    $taskSelection.actual_checkpoint_iteration -ne $taskSelectedIteration -or
    $taskSelection.actual_adam_step -ne $taskSelectedAdam -or
    $taskSelection.effective_updates -ne $taskEffectiveUpdates -or
    $taskSelection.effective_environment_steps -ne (128 * 24 * $taskEffectiveUpdates) -or
    $taskSelection.original_verified_updates -ne 200 -or
    $taskSelection.original_training_receipt_sha256 -ne (Get-FileHash -LiteralPath $CandidateTrainingResult).Hash -or
    $taskSelection.model_tensor_count -ne 17 -or $taskSelection.adam_state_count -ne 17 -or
    $taskSelection.all_model_and_adam_tensors_finite -ne $true -or
    $taskSelection.additional_training_updates -ne 0 -or $taskSelection.receipt_rewritten -ne $false -or
    $taskSelection.selected_is_final_checkpoint -ne $false -or $taskSelection.promotion_performed -ne $false -or
    $taskSelection.recovery_acceptance_proven -ne $false -or $taskSelection.independent_generalization_proven -ne $false) {
    throw 'Retrospective selected-checkpoint provenance mismatch.'
}
if (($taskSelection.original_formal_training | ConvertTo-Json -Depth 8 -Compress) -cne
    ($taskReport.candidate_training | ConvertTo-Json -Depth 8 -Compress)) {throw 'Contradictory original training evidence.'}
...
Write-Output "DIAGNOSTIC COMPLETE startup=$Mode mirror=$MirrorMode pose=$Pose final=$($taskReport.results.$Pose.final_valid_stands)/$Trials. Not acceptance or promotion."
