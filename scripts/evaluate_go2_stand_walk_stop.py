...
def _phase(t: float) -> tuple[str, tuple[float, float, float]]:
    ...
    if t < args_cli.stand_s + args_cli.walk_s:
        return "walk", (args_cli.walk_speed, 0.0, 0.0)
...
def _annotate(frame: np.ndarray, phase: str, t: float, row: dict[str, float]) -> np.ndarray:
    ...
    lines = (
        ...
        f"t={t:5.2f}s cmd_x={row['cmd_x']:+.2f} m/s",
...
def main() -> None:
    ...
    fields = [
        ...
        "vx", "vy", "vx_b", "vy_b", "push_delta_vy", "joint_rms_deviation",
    ]
    ...
    report = {
        ...
        "protocol": {"stand_s": args_cli.stand_s, "walk_s": args_cli.walk_s, "stop_s": args_cli.stop_s, "walk_speed": args_cli.walk_speed, "push_delta_vy": args_cli.push_speed},
