param(
    ...
    [Parameter(Mandatory=$true)][ValidateSet('hard','raw_ramp')][string]$Mode,
    [Parameter(Mandatory=$true)][ValidateSet(0,0.2,0.5)][double]$RampSeconds,
...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_handoff_transition.py'
...
if (($Mode -eq 'hard' -and $RampSeconds -ne 0) -or ($Mode -eq 'raw_ramp' -and $RampSeconds -eq 0)) {
    throw 'hard requires T=0; raw_ramp requires T=0.2 or 0.5. No retry factor is included.'
}
...
$taskRampText = $RampSeconds.ToString('0.0############',[Globalization.CultureInfo]::InvariantCulture)
$taskArgs = @('-B',$taskEntry,'--task',$taskName,'--checkpoint',$Checkpoint,
    '--stand_checkpoint',$StandCheckpoint,'--output_dir',$OutputDir,'--transition_mode',$Mode,
    '--ramp_seconds',$taskRampText,'--trials',"$Trials",'--seed',"$Seed",'--poses',$Pose,
...
$taskInvocation = @{schema='handoff_transition_invocation_v1';started_at=(Get-Date).ToString('o');
    mode=$Mode;ramp_seconds=$RampSeconds;pose=$Pose;trials=$Trials;seed=$Seed;args=$taskArgs;
...
if ($taskReport.protocol_version -ne 'handoff_transition_experimental_v1' -or
    ...
    $taskReport.transition_experiment.mode -ne $Mode -or
    $taskReport.transition_experiment.ramp_seconds -ne $RampSeconds -or
...
$taskExpectedTrace=(Split-Path $Checkpoint -Leaf)+'_handoff_transition_trace.json'
...
if ($taskTraceReport.schema -ne 'handoff_transition_neighborhood_v1' -or
...
Write-Output "DIAGNOSTIC COMPLETE mode=$Mode T=$RampSeconds pose=$Pose final=$($taskReport.results.$Pose.final_valid_stands)/$Trials. Not acceptance or promotion."
