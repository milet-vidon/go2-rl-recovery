...
$taskEntry = Join-Path $PSScriptRoot 'evaluate_natural_handoff_candidate.py'
...
$taskInvocation = @{schema='natural_stance_candidate_invocation_v1';started_at=(Get-Date).ToString('o');
...
if ($taskReport.protocol_version -ne 'natural_stance_candidate_combined_v1' -or
...
if ($taskReport.candidate_training.checkpoint_sha256 -ne (Get-FileHash -LiteralPath $StandCheckpoint).Hash -or
    ...
    $taskReport.candidate_training.verified_updates -ne 200) {throw 'Formal natural candidate provenance mismatch.'}
