...
BASELINE_EVALUATOR_SHA256 = "4024ba713e3ed616225d64e1d59de97fd31c8369bba83ac3f4235d93552e9e2f"
BASELINE_EVALUATOR_PATH = Path(__file__).with_name("evaluate_go2_recovery.py")
BASELINE_TRANSITION_PATH = Path(__file__).with_name("evaluate_handoff_transition.py")
BASELINE_TRANSITION_SHA256 = "c3a433568561fbe0893d61a570cf420304d44dcf986440e8d7efebd85ff7f10a"
BASELINE_TRANSITION_MATH_PATH = Path(__file__).resolve().parents[1] / "src" / "go2_recovery" / "handoff_transition_math.py"
BASELINE_TRANSITION_MATH_SHA256 = "b66167039b4698d66f3cc412b19bacd0b1b2f37279f7ce0856ec6c09b196aa4e"
ROLL_CHECKPOINT_SHA256 = "71e4c7a39ff81836d2ead1c4533988c696e5bd4a0fc939b8cef6b4f6daec1b1c"
STAND_CHECKPOINT_SHA256 = "5527c4053cb6ef8e6616da65b7dc32a9464eb8d1366f4966a4eb4b65bc913fdb"


def _validate_transition_cli(args):
    """Read-only pre-AppLauncher checks; wrapper-created invocation/log may already exist."""
    if args.transition_mode != "hard" or args.ramp_seconds != 0:
        raise ValueError("Mirror diagnostic requires hard handoff with no ramp")
    if args.mirror_mode not in ("off", "initial_right"):
        raise ValueError("Mirror mode must be off or initial_right")
    if args.state_bank_split != "heldout":
        raise ValueError("Experimental transition entry forbids TRAIN collection")
    if args.trials not in (1, 2, 20):
        raise ValueError("Only bounded 1/2/20-trial diagnostics are allowed")
    if args.horizon_s != 8.0 or args.hold_s != 3.0 or args.min_contacts != 4 or args.settle_s != 1.0:
        raise ValueError("Keep horizon=8, hold=3, min_contacts=4 and settle=1 unchanged")
    if args.output_dir.resolve().drive.upper() != "E:":
        raise ValueError("Output must be on E:")
    if args.output_dir.exists():
        if not args.output_dir.is_dir() or any(
            path.name not in ("invocation.json", "simulator.log") or not path.is_file()
            for path in args.output_dir.iterdir()
        ):
            raise ValueError("Output must be new or contain only wrapper invocation.json/simulator.log")
    if hashlib.sha256(BASELINE_EVALUATOR_PATH.read_bytes()).hexdigest() != BASELINE_EVALUATOR_SHA256:
        raise ValueError("Frozen baseline evaluator SHA mismatch")
    for path, expected in ((BASELINE_TRANSITION_PATH, BASELINE_TRANSITION_SHA256),
                           (BASELINE_TRANSITION_MATH_PATH, BASELINE_TRANSITION_MATH_SHA256),
                           (args.checkpoint, ROLL_CHECKPOINT_SHA256),
                           (args.stand_checkpoint, STAND_CHECKPOINT_SHA256)):
        if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != expected:
            raise ValueError(f"Frozen source/checkpoint SHA mismatch: {path}")


def _write_json_new(path, value):
    """Persist before SimulationApp.close, which may terminate Windows with exit zero."""
...
parser.add_argument("--stand_checkpoint", required=True, type=Path,
                    help="Required frozen standing actor; experimental two-policy diagnostic only.")
parser.add_argument("--mirror_mode", required=True, choices=("off", "initial_right"))
parser.set_defaults(transition_mode="hard", ramp_seconds=0.0)
...
try:
    _validate_transition_cli(args_cli)
...
def _annotate(frame: np.ndarray, pose_class: str, step: int, dt: float, stable_s: float, row: dict,
    ...
    if getattr(args_cli, "stand_checkpoint", None):
        ...
        labels = (f"{heading} | {pose_class} | {phase}",) + labels[1:3] + (
    ...
    for i, text in enumerate(labels):
        ...
        status_color = (0, 190, 255)  # Experimental evidence, never a green acceptance marker.
...
def _transition_physical_rows(env, foot_ids):
    """Read current control-boundary state; no stepping, resetting or sensor history replacement."""
    a = env.scene["robot"].data
    sensor = env.scene.sensors["contact_forces"]
    base_ids = sensor.find_bodies("base")[0]
    gravity = a.projected_gravity_b
    tilt = torch.rad2deg(torch.acos((-gravity[:, 2] / gravity.norm(dim=-1).clamp_min(1e-12)).clamp(-1, 1)))
    values = {
        "root_position_local_m": a.root_pos_w - env.scene.env_origins,
        "root_quaternion_wxyz": a.root_quat_w,
        "root_linear_velocity_w_m_s": a.root_lin_vel_w,
        "root_angular_velocity_w_rad_s": a.root_ang_vel_w,
        "joint_positions_rad": a.joint_pos,
        "joint_velocities_rad_s": a.joint_vel,
        "tilt_deg": tilt,
        "height_m": a.root_pos_w[:, 2] - env.scene.env_origins[:, 2],
        "angular_speed_rad_s": a.root_ang_vel_b.norm(dim=-1),
        "current_foot_forces_w_n": sensor.data.net_forces_w[:, foot_ids],
        "current_base_forces_w_n": sensor.data.net_forces_w[:, base_ids],
        "current_base_contact": (sensor.data.net_forces_w[:, base_ids].norm(dim=-1) > 1).any(1),
        "applied_torque_joint_abs_max_at_control_boundary_nm": a.applied_torque.abs().amax(dim=-1),
    }
    if any(not bool(torch.isfinite(value).all()) for value in values.values()):
        raise RuntimeError("Nonfinite state/force/torque in transition diagnostic")
    cpu = {key: value.detach().cpu().tolist() for key, value in values.items()}
    return [{key: value[i] for key, value in cpu.items()} for i in range(env.num_envs)]


def _transition_rows_before(env, foot_ids, obs, step, gate_steps, switched, just_switched,
                            roll_action, stand_action, action, anchor, alpha, target, neighborhood):
    """Snapshot real inputs only for bounded per-trial neighborhoods; never policy replay."""
    selected = [i for i in range(env.num_envs) if neighborhood.wants_row(i, step)]
    if not selected:
        return {}
    a = env.scene["robot"].data
    physical = _transition_physical_rows(env, foot_ids)
    values = {
        "policy_observation": obs["policy"],
        "roll_actor_raw_action": roll_action, "stand_actor_raw_action": stand_action,
        "previous_raw_action": env.action_manager.action,
        "previous_previous_raw_action": env.action_manager.prev_action,
        "velocity_command_b": env.command_manager.get_command("base_velocity"),
        "anchor_raw_action": anchor, "issued_raw_action": action,
        "previous_executed_joint_target_rad": a.joint_pos_target,
        "expected_executed_joint_target_rad": target,
        "joint_target_delta_rad": target - a.joint_pos_target,
        "clamp_mask": ((a.default_joint_pos + .25 * action < a.soft_joint_pos_limits[:, :, 0]) |
                       (a.default_joint_pos + .25 * action > a.soft_joint_pos_limits[:, :, 1])),
        "alpha": alpha, "gate_steps": gate_steps, "switched": switched, "just_switched": just_switched,
    }
    if values["policy_observation"].shape != (env.num_envs, 48):
        raise RuntimeError("Transition diagnostic requires the actual 48-dimensional policy observation")
    if any(not bool(torch.isfinite(value).all()) for value in values.values()):
        raise RuntimeError("Nonfinite transition policy inputs/actions/targets")
    cpu = {key: value.detach().cpu().tolist() for key, value in values.items()}
    rows = {}
    for i in selected:
        row = {key: value[i] for key, value in cpu.items()}
        row.update(trial=i, step=step, policy_time_s=step * env.step_dt, before=physical[i],
                   phase=("roll" if not row["switched"] else "raw_ramp" if row["alpha"] < 1 else "stand"))
        rows[i] = row
    return rows


def _validate_mirror_observation_contract(env, actors):
    """Fail closed on semantic layout changes; never normalize/rebuild actor inputs."""
    from roll_mirror_math import OBSERVATION_TERMS
    manager = env.observation_manager
    if tuple(manager.active_terms["policy"]) != OBSERVATION_TERMS or tuple(manager.group_obs_dim["policy"]) != (48,):
        raise RuntimeError("Mirror requires the verified seven-term native 48 observation layout")
    if list(manager.group_obs_term_dim["policy"]) != [(3,), (3,), (3,), (3,), (12,), (12,), (12,)]:
        raise RuntimeError("Mirror policy observation term dimensions changed")
    group = env.cfg.observations.policy
    if group.enable_corruption or not group.concatenate_terms:
        raise RuntimeError("Mirror diagnostics require real deterministic uncorrupted observations")
    expected_functions = ("base_lin_vel", "base_ang_vel", "projected_gravity", "generated_commands",
                          "joint_pos_rel", "joint_vel_rel", "last_action")
    for name, expected in zip(OBSERVATION_TERMS, expected_functions):
        term = getattr(group, name)
        if term.func.__name__ != expected or term.clip is not None or term.modifiers:
            raise RuntimeError(f"Changed mirror observation function/clip/modifiers: {name}")
        if term.history_length != 0:
            raise RuntimeError("Stacked observation history is not the verified 48-input contract")
        if term.scale is not None and not bool(torch.as_tensor(term.scale).eq(1).all()):
            raise RuntimeError(f"Unexpected observation scaling: {name}")
    for actor in actors:
        if actor.actor_obs_normalization or not isinstance(actor.actor_obs_normalizer, torch.nn.Identity):
            raise RuntimeError("Frozen mirror diagnostic does not support normalized actor inputs")
        if tuple(actor.obs_groups["policy"]) != ("policy",) or actor.is_recurrent:
            raise RuntimeError("Require a feed-forward actor consuming only the policy48 group")
    a = env.scene["robot"].data
    if not bool((a.default_joint_vel == 0).all()):
        raise RuntimeError("Expected zero default joint velocities")
    term = env.action_manager.get_term("joint_pos")
    if tuple(term._joint_names) != tuple(env.scene["robot"].joint_names):
        raise RuntimeError("Action term order differs from native joint order")


def _assert_real_observation_contract(env, obs):
    """Check the stored real48 against runtime buffers; never use this as policy input."""
    a = env.scene["robot"].data
    measured = torch.cat((a.root_lin_vel_b, a.root_ang_vel_b, a.projected_gravity_b,
                          env.command_manager.get_command("base_velocity"),
                          a.joint_pos - a.default_joint_pos, a.joint_vel - a.default_joint_vel,
                          env.action_manager.action), dim=-1)
    if obs["policy"].shape != measured.shape or not torch.equal(obs["policy"], measured):
        raise RuntimeError("Actual policy48 differs from the verified native measurement layout")
...
def main() -> None:
    ...
    if args_cli.stand_checkpoint:
        ...
        from handoff_transition_math import ramp_step_count, transition_action, TransitionNeighborhood
        from roll_mirror_math import (initial_right_mask, roll_input_copy, physical_roll_action,
                                      validate_mirror_joint_contract, RIGHT_GRAVITY_Y_THRESHOLD)
        ...
        mirror_joint_contract = validate_mirror_joint_contract(
            env.unwrapped.scene["robot"].joint_names, a.default_joint_pos,
            a.soft_joint_pos_limits, a.joint_pos_limits)
    ...
    actual_step_dt = env.unwrapped.step_dt
    ramp_steps = ramp_step_count(args_cli.ramp_seconds, actual_step_dt)
    neighborhood_steps = round(1.0 / actual_step_dt)
    if actual_step_dt != 0.02:
        raise RuntimeError("This bounded experiment requires the unchanged 50 Hz control loop")
    ...
    try:
        ...
        for pose_class in args_cli.poses:
            ...
            # One fixed mask from REAL policy-start measurements. No requested
            # pose, source ID or outcome participates; no per-frame reclassification.
            ...
            normalized_start_y = start_gravity[:, 1] / start_gravity.norm(dim=-1)
            mirror_selection_by_pose[pose_class] = [{
                "trial": i, "selected": bool(mirror_selected[i]),
                "eligible_settled_fallen_at_policy_start": bool(eligible_fallen[i]),
                "real_policy_start_projected_gravity_b": start_gravity[i].cpu().tolist(),
                "normalized_policy_start_gravity_y": float(normalized_start_y[i]),
                "strict_gravity_y_threshold": RIGHT_GRAVITY_Y_THRESHOLD,
                "mask_latched_for_episode": True,
            } for i in range(env.num_envs)]
            obs = env.get_observations()
            _assert_real_observation_contract(env.unwrapped, obs)
            ...
            if stand_policy is not None:
                ...
                ramp_anchor = torch.zeros_like(env.unwrapped.scene["robot"].data.joint_pos)
                ramp_progress = torch.zeros(env.num_envs, device=env.device, dtype=torch.long)
            ...
            for step in range(steps):
                ...
                with torch.no_grad():
                    if stand_policy is None:
                        ...
                        _assert_real_observation_contract(env.unwrapped, obs)
                        real_policy_observation_before = obs["policy"].clone()
                        # Deep clone all TensorDict leaves; never put virtual
                        # measurements into the real obs/action-manager buffers.
                        roll_obs = roll_input_copy(obs, mirror_selected)
                        roll_model_action = policy(roll_obs)
                        roll_action = physical_roll_action(roll_model_action, mirror_selected)
                        stand_action = stand_policy(obs)  # ALWAYS real/native observations.
                        if not torch.equal(obs["policy"], real_policy_observation_before):
                            raise RuntimeError("Real observation was mutated by actor-interface diagnostic")
                        previous_raw_action = env.unwrapped.action_manager.action.clone()
                        action, ramp_anchor, ramp_progress, ramp_alpha = transition_action(
                        ...
                        for i in just_switched.nonzero(as_tuple=False).flatten().cpu().tolist():
                            # Real pre-transition state. In ramp mode the issued action is
                            # interpolated explicitly; state/history/PD are never replaced.
                            switch_records[i] = {
                                ...
                                "roll_mirror_selected": bool(mirror_selected[i]),
                                "real_policy_observation": real_policy_observation_before[i].cpu().tolist(),
                                "roll_policy_input_observation": roll_obs["policy"][i].cpu().tolist(),
                                "roll_actor_output_model_raw_action": roll_model_action[i].cpu().tolist(),
                                "roll_actor_output_physical_raw_action": roll_action[i].cpu().tolist(),
                        ...
                        transition_rows = _transition_rows_before(
                            env.unwrapped, foot_ids, obs, step, gate_steps, switched, just_switched,
                            roll_action, stand_action, action, ramp_anchor, ramp_alpha, target, neighborhood)
                        for i, row in transition_rows.items():
                            row.update(roll_mirror_selected=bool(mirror_selected[i]),
                                real_policy_observation=real_policy_observation_before[i].cpu().tolist(),
                                roll_policy_input_observation=roll_obs["policy"][i].cpu().tolist(),
                                roll_actor_output_model_raw_action=roll_model_action[i].cpu().tolist(),
                                roll_actor_output_physical_raw_action=roll_action[i].cpu().tolist())
                        obs, _, dones, _ = env.step(action)
                        # Read-only assertions every step, including all trials outside
                        # the stored neighborhood. History advances only through env.step.
                        if not torch.equal(env.unwrapped.action_manager.action, action):
                            raise RuntimeError("Actual raw action differs from issued transition action")
                        if not torch.equal(env.unwrapped.action_manager.prev_action, previous_raw_action):
                            raise RuntimeError("Action history did not naturally advance through env.step")
                        ...
                        if transition_rows:
                            physical_after = _transition_physical_rows(env.unwrapped, foot_ids)
                            actual_targets = a.joint_pos_target.detach().cpu().tolist()
                            actual_actions = env.unwrapped.action_manager.action.detach().cpu().tolist()
                            actual_history = env.unwrapped.action_manager.prev_action.detach().cpu().tolist()
                            for i, row in transition_rows.items():
                                row.update(after=physical_after[i], actual_executed_joint_target_rad=actual_targets[i],
                                           actual_raw_action=actual_actions[i], actual_previous_raw_action=actual_history[i],
                                           issued_action_target_history_assertions_passed=True)
            ...
            if stand_policy is not None:
                transition_trace_by_pose[pose_class] = neighborhood.report()
                for trial_trace, selected in zip(transition_trace_by_pose[pose_class], mirror_selection_by_pose[pose_class]):
                    trial_trace["mirror_selection"] = selected
                results[pose_class]["mirror_diagnostic"] = {
                    "selected_trials": int(mirror_selected.sum()),
                    "total_trials": int(env.num_envs),
                    "selection_records": mirror_selection_by_pose[pose_class],
                    "all_trials_in_success_denominator": True,
                }
    ...
    transition_path = args_cli.output_dir / f"{checkpoint_name}_handoff_mirror_trace.json"
    math_path = Path(__file__).resolve().parents[1] / "src" / "go2_recovery" / "handoff_transition_math.py"
    experiment = {
        "mode": args_cli.transition_mode, "ramp_seconds": args_cli.ramp_seconds,
        "step_dt": actual_step_dt, "ramp_steps": ramp_steps,
        "horizon_s": args_cli.horizon_s, "hold_s": args_cli.hold_s,
        "min_contacts": args_cli.min_contacts, "policy_control_steps": steps,
        "policy_duration_including_hold_s": steps * actual_step_dt,
        "unchanged_one_way_gate": True,
        "state_or_history_reset_at_switch": False, "hidden_pd_at_switch": False,
        "retry_enabled": False,
        "equation": "k=1..N: (1-k/N)*previous-issued-raw-at-edge + (k/N)*current-stand-actor; exact stand after N",
        "hard_zero_semantics": "exact original torch.where(switched[:,None], stand_action, roll_action)",
        "actual_action_and_history_asserted_every_step": True,
        "actual_joint_target_asserted_every_step_atol_rad": 1e-6,
        "evaluator_source_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "math_source_sha256": hashlib.sha256(math_path.read_bytes()).hexdigest(),
        "baseline_evaluator_path": str(BASELINE_EVALUATOR_PATH.resolve()),
        "baseline_evaluator_sha256": BASELINE_EVALUATOR_SHA256,
        "transition_trace_file": transition_path.name,
        "engineering_diagnostic_not_paper_reproduction": True,
        "no_standing_retention_acceptance": True,
        "known_baseline_caveat": "Stand3547-only upright20/20 is NOT dual-policy upright. Frozen old dual1999-to3547 cold controls were upright12/20, side9/20, upside_down20/20; hard equivalence must be measured, never assumed.",
        "heldout_reuse_caveat": "Repeatedly inspected bank cases are development evidence, not a fresh generalization test.",
    }
    report["baseline_metric_protocol_version"] = report["protocol_version"]
    report["baseline_transition_protocol_version"] = "handoff_transition_experimental_v1"
    ...
    report["single_policy_acceptance_eligible"] = False
    report["training_collection_eligible"] = False
    report["transition_experiment"] = experiment
    mirror_math_path = Path(__file__).resolve().parents[1] / "src" / "go2_recovery" / "roll_mirror_math.py"
    mirror_experiment = {
        "mode": args_cli.mirror_mode,
        "selection_rule": "eligible_settled_fallen_at_real_policy_start AND normalized_projected_gravity_body_y < -cos(30deg); mode off selects none; no requested pose/ID/outcome input",
        "normalized_gravity_y_threshold": RIGHT_GRAVITY_Y_THRESHOLD,
        "fixed_mask_per_episode": True, "roll_only": True,
        "standing_uses_real_observation": True, "gate_uses_real_state": True,
        "physics_or_history_mutated": False,
        "ramp_enabled": False, "retry_enabled": False, "startup_selection_changed": False,
        "mode_does_not_guarantee_upright_retention": True,
        "roll_equation": "selected: J12(roll_actor(M48(real_obs))); unselected: roll_actor(real_obs); stand_actor always real_obs",
        "tensor_dict_copy": "clone(recurse=True), then assign a newly allocated masked policy tensor; never a shallow clone or a write into real observations",
        "network_normalization": "both actual actors must expose Identity actor_obs_normalizer and actor_obs_normalization=False",
        "observation_field_semantics": {
            "policy_observation": "real measured native48 (legacy field)",
            "real_policy_observation": "explicit duplicate of real measured native48",
            "roll_policy_input_observation": "actual48 passed to roll actor, virtual reflection only for fixed selected rows",
            "roll_actor_output_model_raw_action": "roll actor output before inverse reflection, model coordinates",
            "roll_actor_output_physical_raw_action": "native real12 after inverse reflection for selected rows",
            "roll_actor_raw_action": "same physical/native roll candidate as preceding field (legacy name)",
            "stand_actor_raw_action": "standing actor from unmodified real48",
        },
        "joint_mapping_contract": mirror_joint_contract,
        "real_policy48_asserted_against_runtime_every_step": True,
        "step_dt": actual_step_dt, "horizon_s": args_cli.horizon_s, "hold_s": args_cli.hold_s,
        "min_contacts": args_cli.min_contacts, "policy_control_steps": steps,
        "evaluator_source_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "math_source_sha256": hashlib.sha256(mirror_math_path.read_bytes()).hexdigest(),
        "baseline_transition_evaluator_path": str(BASELINE_TRANSITION_PATH.resolve()),
        "baseline_transition_evaluator_sha256": BASELINE_TRANSITION_SHA256,
        "baseline_transition_math_sha256": BASELINE_TRANSITION_MATH_SHA256,
        "mirror_trace_file": transition_path.name,
    ...
    _write_json_new(transition_path, {
        ...
        "acceptance_eligible": False, "training_collection_eligible": False,
        "transition_experiment": experiment,
        "mirror_experiment": mirror_experiment,
        "checkpoint_sha256": report["checkpoint_sha256"],
        "stand_checkpoint_sha256": report["handoff_controller"]["stand_sha256"],
        "neighborhood_before_steps": neighborhood_steps,
        "neighborhood_after_steps": neighborhood_steps,
        "coverage": "All trials and first edges; bounded pre-edge ring plus switch through +1s. Untriggered trials retain their last1s and remain in every denominator.",
        "frames_and_timing": "Root LINK position subtracts env origin; quaternion wxyz; CoM linear/angular velocities world; joint and raw-action vectors native joint order; forces world. Before/after are immediately before/after the single natural control step. Applied torque is max across joints at that control boundary, NOT a maximum across physics substeps.",
        "not_a_replay_bank": "Trace is diagnostic only, not a validated reset archive; PhysX contact/solver caches cannot be restored from these fields.",
        "joint_names": joint_names, "poses": transition_trace_by_pose,
    })
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.flush()
        args_cli.output_dir.mkdir(parents=True, exist_ok=True)
        _write_json_new(args_cli.output_dir / "transition_error.json", {
            "protocol_version": "handoff_mirror_experimental_v1",
            "completed": False, "acceptance_eligible": False,
