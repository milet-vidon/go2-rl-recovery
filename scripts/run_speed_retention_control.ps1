param(
    [ValidatePattern('^[A-Za-z0-9_-]+$')][string]$RunTag = '20260917-speedretention128x300',
    [ValidateSet(2,300)][int]$Iterations = 300,
    [ValidateSet(16,128)][int]$NumEnvs = 128,
    [switch]$PreflightOnly
)
# Finite original-distribution continuation CONTROL, TRAINING ONLY.
# Same parent/budget/LR as the failed expanded-range pilot, no reward intervention.
# No promotion, automatic extension, or modification of existing scripts/models.
$ErrorActionPreference = 'Stop'
if (-not (($NumEnvs -eq 16 -and $Iterations -eq 2) -or ($NumEnvs -eq 128 -and $Iterations -eq 300))) {
    throw 'Use 16 envs/2 updates for smoke or 128 envs/300 updates for the formal control.'
}
...
$parentConfigHashes = @{
    'env.yaml'='12641D59DD5432FB54148FE9BBF073445849C6CDCD4C02EFDF41AE022FDA7E3E'
    'agent.yaml'='1C56097D934E1A404933A81E7B7167C48BDDF39F099C73F16F69D8AB2085EAE5'
}
...
function Test-IsaacPythonOrKit($Process) {
    # Intentionally conservative: all workspace Python/Kit processes block launch,
    # including CPU validators. No filename whitelist can miss a future handoff,
    # reset, replay, training or simulator entry script. This never stops processes.
    return ($Process.Name -match '^(python.*|kit|isaac-sim.*)\.exe$' -and
            ($Process.CommandLine -match 'E:[/\\]IsaacLab(?:[/\\]|\b)' -or
             $Process.ExecutablePath -match '^E:[/\\]IsaacLab(?:[/\\]|\b)'))
}
function Assert-Idle {
    $active = @(Get-CimInstance Win32_Process | Where-Object { Test-IsaacPythonOrKit $_ })
    if ($active.Count) {
        throw "Workspace Python/Kit process active: $($active.ProcessId -join ','). This conservative guard also blocks CPU validators; wait, do not overlap."
    }
}
...
foreach ($path in @($logRoot,$config)) {
    if (Test-Path -LiteralPath $path) { throw "Existing output: $path. Preserve evidence and never rerun over it." }
}
...
if ((Get-FileHash -LiteralPath $parent -Algorithm SHA256).Hash -ne $parentSha) { throw 'Frozen parent checkpoint SHA256 mismatch.' }
...
foreach ($name in @('env.yaml','agent.yaml')) {
    ...
    foreach ($path in @($actual,$archive)) {
        if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash -ne $parentConfigHashes[$name]) { throw "Frozen parent config mismatch: $path" }
        $sources += @{path=$path;sha256=$parentConfigHashes[$name]}
    }
}
...
foreach ($path in @((Join-Path $PSScriptRoot 'train.ps1'),$PSCommandPath,
                    (Join-Path $PSScriptRoot 'check_speed_retention_config.py'),
...
Write-Output "Original-distribution CONTROL, TRAINING ONLY: parent3648; $NumEnvs envs/$Iterations updates, $($NumEnvs * $Iterations * 24) environment steps; expected $expectedFinal."
Write-Output 'Keep vx[-0.5,1.0],30% standing and all old rewards/commands/pushes. Match saved LR scalar1e-5; full optimizer, not exact RNG/simulator continuity.'
...
try {
    ...
    & $python ./scripts/check_speed_retention_config.py $config --num-envs $NumEnvs --iterations $Iterations --run-name $RunTag --checkpoint $checkpoint *> $checkLog
    if ($LASTEXITCODE -ne 0) { throw "Control config/checkpoint guard failed: $checkLog" }
    Assert-Sources
    Get-Content -LiteralPath $checkLog
    Write-Output "Finite control TRAINING ONLY complete: $checkpoint; SHA256 $((Get-FileHash -LiteralPath $checkpoint -Algorithm SHA256).Hash)."
    Write-Output 'No promotion or automatic extension. Compare the full old-function/target-speed regression before deciding what changed.'
}
finally { Pop-Location }
