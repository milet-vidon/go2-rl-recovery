...
if (-not $taskCheckpoint.StartsWith('E:\IsaacLab\repo\logs\rsl_rl\unitree_go2_flat\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path $taskCheckpoint -Leaf) -cne 'model_4046.pt') {throw 'Only isolated formal candidate4046.'}
$taskCheckpointSha = (Get-FileHash -LiteralPath $taskCheckpoint -Algorithm SHA256).Hash.ToLowerInvariant()
$taskEntry = Join-Path $PSScriptRoot 'evaluate_common_physics_candidate.py'
...
& $taskPython -B $taskEntry --candidate_training_result $TrainingResult --checkpoint $taskCheckpoint --verify-only
...
$taskArgs = @('-B',$taskEntry,'--candidate_training_result',$TrainingResult,'--physics_mode',$PhysicsMode,'--task',$taskName,'--checkpoint',$taskCheckpoint,
...
$taskInvocation = [ordered]@{schema='trained_common_physics_candidate_invocation_v1';status='running';
    started_at=(Get-Date).ToString('o');physics_mode=$PhysicsMode;seed=$Seed;args=$taskArgs;
...
try {
    ...
    Assert-RetentionIntegrity
    $taskReportPath = Join-Path $OutputDir 'model_4046_stand_walk_stop.json'
    ...
    if ($taskReport.candidate_training.checkpoint_sha256 -ne $taskCheckpointSha -or
        $taskReport.candidate_training.formal_updates_verified -ne 100 -or
    ...
    if ($taskTrace.schema -ne 'trained_common_physics_candidate_retention_v1' -or $taskTrace.physics_mode -ne $PhysicsMode -or
    ...
    $taskCsv = Join-Path $OutputDir 'model_4046_stand_walk_stop.csv'
    ...
    if (-not $NoVideo) {
        $taskVideo = Join-Path $OutputDir 'model_4046_stand_walk_stop.mp4'
