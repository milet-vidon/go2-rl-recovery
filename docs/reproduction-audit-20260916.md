...
正在运行的是新128环境配对：两组相同初始网络、相同PPO/奖励/状态库，各2000轮；只比较默认关节角与控制周期开始时当前关节角作为目标参考。其依据是Lee2019的动作设计，但还不是论文完整复现，也不能预先断言一定奏效。新旧并行环境数量不同，不能把两批训练合并为同一控制变量实验。
...
1. 完成当前128环境动作参考配对及相同协议的确定性测试；不能因奖励高或随机采样偶然翻身就晋升。
