...
## Robust-push continuation (2026-09-09)

The push-focused task `Isaac-Natural-Robust-Push-Flat-Unitree-Go2-v0` continued `model_2849.pt` for 800 PPO iterations with 512 environments (9,830,400 environment steps). It keeps the 48/12 observation-action layout and adds no scripted recovery. The training event uses Isaac Lab's `push_by_setting_velocity`, which samples and adds a bounded root velocity; it is a velocity impulse proxy, not a calibrated force or torque push. The new variant uses x/y ranges of ±0.9 m/s and stronger static-stance and flat-orientation terms.

Checkpoint `model_3648.pt` passed the complete no-disturbance stand/walk/stop rollout on seed 20260909 (walk tilt 3.84°, walk speed 0.467 m/s, diagonal support 0.929, stop speed P95 0.006 m/s, four-foot rest fraction 1.0). Three-seed normal/left/right/fast maneuver regressions also passed. Under the stronger `PushSpeed=0.75` diagnostic, all tested seeds had no reset and no base contact, but the settled stop four-foot fraction remained below 0.95 (seed 20260909: 0.92); this continuation is therefore experimental and not promoted over `natural_stop_diagonal_model2250.pt`.

Artifacts: `evaluations/robust-push-3648-video/` and `evaluations/robust-push-3648-video-push/`. The latter video shows the transient unloading that explains the strict failed screen. The task overlay is registered separately so prior runs remain reproducible.
...
## Diagonal symmetry follow-up (2026-09-09)

The first symmetry run added a locomotion-gated foot-swing symmetry term and screened checkpoint 1948. The implementation was then corrected to compare the trot diagonals (`FL↔RR` and `FR↔RL`) rather than same-side front/rear feet. Run `2026-09-09_03-42-13_natural_stop_diagonal_20260909` continued for 400 PPO iterations with 512 environments (about 4,915,200 additional environment steps). Checkpoint `model_2250.pt` was selected over `model_2347.pt` after the same fixed protocol on three seeds.

| Seed | Passed | Walk max tilt | Walk speed | Diagonal support | Stop speed P95 |
| --- | --- | ---: | ---: | ---: | ---: |
| 20260909 | yes | 4.77° | 0.554 m/s | 0.886 | 0.011 m/s |
| 20260910 | yes | 4.92° | 0.560 m/s | 0.943 | 0.020 m/s |
| 20260911 | yes | 5.05° | 0.540 m/s | 0.930 | 0.000 m/s |

The selected model also passed the bounded lateral impulse test with `PushSpeed=0.5`: no reset, no base contact, maximum roll/pitch 4.76°/5.40°, and 0.996 four-foot contact fraction in the settled stop phase. This is a velocity-impulse simulation, not a calibrated physical shove. The video and raw data are in `evaluations/diagonal-2250-video/`; the model is `models/locomotion/natural_stop_diagonal_model2250.pt`.

This improves the flat-ground stand/walk/stop baseline but does not add AMP, terrain curriculum, arbitrary initial-pose recovery, or hardware validation. The reference video therefore remains a research target rather than a fully reproduced result.

## Recovery checkpoint audit (2026-09-09)

The previously selected recovery checkpoint `recovery_stage30_model2200.pt` was re-evaluated with 20 deterministic trials per pose class and a required 3 s stable hold. At 15° starts it recovered 13/20 side falls and 15/20 fore/aft falls; median recovery times were 0.62 s and 0.56 s. At 30° it recovered 4/20 side and 10/20 fore/aft trials. At 45° it recovered 0/20 side and 4/20 fore/aft trials. These tests use controlled-drop starts generated from the conservative default-pose envelope; they are not arbitrary settled falls or hardware tests.

The later Stable Recovery `model_2799.pt` was not promoted: its 30° side and fore/aft success rates were both 0/20. This regression shows that extending training can trade away learned self-righting behavior. Reports are preserved in `evaluations/recovery-stage30-2200-angle15/`, `evaluations/recovery-stage30-2200-angle30/`, `evaluations/recovery-stage30-2200-angle45/`, and `evaluations/recovery-stable-2799-angle30/`.

A controlled 200-iteration continuation from `model_2250.pt` produced `model_2449.pt` (about 2,457,600 additional environment steps). It still passed the single-seed screen, but its settled walking tilt rose to 6.48 degrees, so it was deliberately not promoted. The raw report is preserved in `evaluations/refine-2449/`; this checkpoint-selection rule prevents longer training from silently replacing a better validated policy.
