...
Status updated16:18: the512-environment run crashed near iteration584 with PhysX CUDA out-of-memory; only the intact500 checkpoint was evaluated (side0/20,back0/20). CurrentTarget512 never started. A NEW128-environment matched pair started16:14,2000 iterations/6,144,000 environment steps per arm, preserving identical fresh initialization within that pair. No new formal result or accepted recovery model yet. Active log `E:/IsaacLab/artifacts/recovery-20260916/target128x2000-orchestrator.log`; [live handoff status](training-status-20260916.md).
