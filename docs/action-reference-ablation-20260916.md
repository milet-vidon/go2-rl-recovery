...
Status: formal pair started2026-09-16 15:52 local after training/evaluation smoke tests. Nominal arm is running first, Current arm queued serially. No formal results yet; not an accepted recovery model. Active log `E:/IsaacLab/artifacts/recovery-20260916/target1000-orchestrator-retry.log`; [live handoff status](training-status-20260916.md).
...
Formal plan: serial512-environment,1000-iteration arms (12,288,000 environment steps each), then strict20-trial side/back bank, upright/30-degree and upright/45-degree evaluations for each arm. The finite launcher is `scripts/run_recovery_action_ablation.ps1`; it checks exact bootstrap/bank hashes, refuses duplicate outputs and does not automatically promote or extend either model. Controlled-drop angle tests use matched fixed Bank physics, NOT the randomized historical Aligned task; do not compare their counts as if identical.
